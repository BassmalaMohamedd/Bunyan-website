...
@php
    $lang = app()->getLocale();
    $title = $page->title[$lang] ?? $page->title['en'] ?? 'Vertex';
    $content = $page->content[$lang] ?? $page->content['en'] ?? '';
@endphp

@section('title', $title . ' | Vertex')
...
@section('content')
    ...
    <style>
        ...
        .hero-bg-reveal { animation: hero-fade-in 2.5s cubic-bezier(0.16, 1, 0.3, 1) forwards; }
    </style>
    ...
    <section style="height:70vh;background:#1c1917;display:flex;align-items:center;justify-content:center;position:relative;overflow:hidden;padding:0 60px;">
        <div class="hero-bg-reveal" style="position:absolute;inset:0;">
            <img src="{{ asset('images/B2.jpg') }}" alt="Vertex" style="width:100%;height:100%;object-fit:cover;opacity:0.25;">
    ...
    <section style="padding:100px 60px;background:#fafaf8;position:relative;">
        <div style="max-width:1440px;margin:0 auto;">
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:0;align-items:center;">
                <div class="reveal-scale" style="height:650px;border-radius:2px;overflow:hidden;box-shadow:0 30px 60px rgba(0,0,0,0.1);z-index:1;position:relative;">
                    <img src="{{ asset('images/B1.jpg') }}" style="width:100%;height:100%;object-fit:cover;">
                </div>
                <div class="reveal" style="background:#fff;padding:80px;border:1px solid #e7e5e4;box-shadow:0 40px 100px rgba(28,25,23,0.06);margin-left:-100px;position:relative;z-index:2;border-left:4px solid #f59e0b;">
                    ...
                    <h2 style="font-family:var(--font-heading);font-size:clamp(2.2rem,3vw,3rem);color:#1c1917;font-weight:900;font-style:italic;line-height:1.2;letter-spacing:-1px;margin-bottom:30px;">
                        Generational wealth through monumental property expansion.
                    </h2>
                    <p style="font-size:1.1rem;line-height:2.1;color:#78716c;font-weight:300;margin-bottom:24px;">
                        At Vertex, we view properties not as static assets, but as dynamic vehicles. We have meticulously built comprehensive real estate ecosystems to transition standard investments into elite, future-proof portfolios.
                    </p>
                    <p style="font-size:1.1rem;line-height:2.1;color:#78716c;font-weight:300;">
                        Guided entirely by unyielding integrity and precision-driven strategies, we proudly power the Kingdom's most ambitious structural developments and commercial achievements.
                    </p>
                </div>
            </div>
        </div>
    ...
    <section style="padding:80px 60px;background:#1c1917;position:relative;overflow:hidden;">
        <div style="max-width:1440px;margin:0 auto;position:relative;z-index:1;">
            <div class="reveal" style="text-align:center;margin-bottom:60px;">
                ...
                <h2 style="font-family:var(--font-heading);font-size:clamp(3rem,4vw,5rem);color:#fff;font-weight:900;font-style:italic;line-height:1.2;letter-spacing:-1px;">
            ...
            <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:2px;background:rgba(245,158,11,0.06);">
                @php
                    $values = [
                        ...
                        ['n'=>'02','t'=>'Market Precision','d'=>'Meticulous attention to detail in every valuation ensures absolute accuracy of all market insights and financial metrics throughout the Kingdom.','icon'=>'M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z M15 12a3 3 0 11-6 0 3 3 0 016 0z'],
                ...
                <div class="reveal" style="transition-delay:{{ $i * 100 }}ms;">
                    <div style="padding:64px 52px;background:#1c1917;height:100%;transition:background 0.7s ease;"
                        ...
                        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:40px;">
                            ...
                            <div style="width:52px;height:52px;background:rgba(245,158,11,0.08);border-radius:12px;display:flex;align-items:center;justify-content:center;">
    ...
    <section style="padding:80px 60px;background:#fff;text-align:center;">
        <div style="max-width:1440px;margin:0 auto;">
            <div class="reveal" style="margin-bottom:60px;">
                ...
                <h2 style="font-family:var(--font-heading);font-size:clamp(3rem,4vw,5rem);color:#1c1917;font-weight:900;font-style:italic;letter-spacing:-1px;">
            ...
            @php
                $pillars = [
                    ...
                    ['t'=>'Asset Optimization','d'=>'Elevating property performance through strategic management, ensuring every square meter maximizes its revenue potential while minimizing operational friction.'],
                ];
            @endphp
            <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:40px;">
                @foreach($pillars as $i => $p)
                <div class="reveal strategy-box" style="text-align:left;background:#fafaf8;border:1px solid #e7e5e4;padding:48px;transition:all 0.4s ease;border-radius:2px;">
                    ...
                    <p style="color:#78716c;font-size:1rem;line-height:2;font-weight:300;">{{ $p['d'] }}</p>
                </div>
                @endforeach
            </div>
        </div>
