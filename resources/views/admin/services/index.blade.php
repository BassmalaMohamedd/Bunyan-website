...
<div class="max-w-7xl mx-auto">
    ...
    <div class="flex justify-between items-end mb-12">
        <div>
            <span class="text-[#f59e0b] font-bold tracking-[0.2em] text-xs uppercase mb-2 block">Specialized Architecture</span>
            <h1 class="text-4xl font-black text-white tracking-tight">Ecosystem Orchestration</h1>
        </div>
        <a href="{{ route('admin.services.create') }}" class="bg-[#f59e0b] hover:bg-opacity-90 text-[#1c1917] px-8 py-4 rounded-xl font-bold transition-all transform hover:-translate-y-1 shadow-lg shadow-amber-500/20 flex items-center gap-3 no-underline">
            ...
            <tbody class="divide-y divide-gray-50 text-[#1c1917]">
                ...
                <tr class="group hover:bg-gray-50/80 transition-all duration-300">
                        ...
                        <div class="flex items-center gap-6">
                            <div class="w-16 h-16 bg-gray-50 rounded-2xl flex items-center justify-center border-2 border-white shadow-sm overflow-hidden flex-shrink-0 ring-4 ring-gray-50/50">
                                @if(is_array($service->image) ? ($service->image['url'] ?? null) : $service->image)
                                    <img src="{{ is_array($service->image) ? $service->image['url'] : $service->image }}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
                                @else
                                    <div class="text-[#f59e0b] font-black text-xl">{{ mb_substr($service->title['en'] ?? 'S', 0, 1) }}</div>
                                @endif
                            </div>
                            <div class="flex flex-col">
                                <span class="font-bold text-[#1c1917] text-lg group-hover:text-[#f59e0b] transition-colors leading-tight">{{ $service->title['en'] ?? 'Untitled Service' }}</span>
                    ...
                    <td class="px-8 py-6">
                        ...
                        @else
                            <span class="bg-amber-50 text-amber-600 px-4 py-1.5 rounded-full text-[0.65rem] font-black uppercase tracking-widest">Maintenance</span>
                        @endif
                    </td>
                    <td class="px-8 py-6 text-right">
                        <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <a href="{{ route('admin.services.edit', $service->id) }}" class="p-3 text-[#1c1917] hover:bg-white hover:shadow-md rounded-xl transition-all border border-transparent hover:border-gray-100">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                            </a>
                            <form action="{{ route('admin.services.destroy', $service->id) }}" method="POST" onsubmit="return confirm('Archive this service module from the ecosystem?')" style="display: inline;">
                                @csrf
                                @method('DELETE')
