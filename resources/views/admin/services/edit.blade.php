...
<div class="max-w-4xl mx-auto">
    <div class="mb-12">
        ...
        <h1 class="text-4xl font-black text-white tracking-tight">Refine Service Modular</h1>
        <p class="text-gray-400 mt-2 font-medium">Update the specialized service specifications in the Vertex ecosystem.</p>
    </div>
    ...
    <form action="{{ route('admin.services.update', $service->id) }}" method="POST" class="space-y-8">
            ...
            <div class="grid grid-cols-1 gap-10 text-[#1c1917]">
                ...
                <div class="grid grid-cols-2 gap-10">
                    <div>
                        ...
                        <input name="slug" type="text" placeholder="property-valuation" value="{{ old('slug', $service->slug) }}" required
                    ...
                    <div>
                        ...
                        <input name="icon" type="text" placeholder="home-valuation-icon" value="{{ old('icon', $service->icon) }}"
                            class="w-full bg-gray-50 border-transparent focus:bg-white focus:border-[#f59e0b] focus:ring-0 rounded-2xl py-4 px-6 text-[#1c1917] font-bold transition-all" />
                ...
                <div class="grid grid-cols-2 gap-10 border-t border-gray-50 pt-10">
                    ...
                    <div class="space-y-6 text-left">
                        ...
                        <div>
                            ...
                            <input name="title[en]" type="text" placeholder="Specialized Valuation" value="{{ old('title.en', $service->title['en'] ?? '') }}" required
                        ...
                        <div>
                            ...
                            <textarea name="description[en]" rows="8" placeholder="Outline the specialized value proposition..."
                                class="w-full bg-gray-50 border-transparent focus:bg-white focus:border-[#f59e0b] focus:ring-0 rounded-2xl py-4 px-6 text-[#1c1917] font-medium transition-all resize-none text-left">{{ old('description.en', $service->description['en'] ?? '') }}</textarea>
                    ...
                    <div class="space-y-6 text-right" dir="rtl">
                        ...
                        <div>
                            ...
                            <input name="title[ar]" type="text" placeholder="تقييم عقاري تخصصي" value="{{ old('title.ar', $service->title['ar'] ?? '') }}" required
                        ...
                        <div>
                            ...
                            <textarea name="description[ar]" rows="8" placeholder="وصف القيمة المضافة..."
                                class="w-full bg-gray-50 border-transparent focus:bg-white focus:border-[#f59e0b] focus:ring-0 rounded-2xl py-4 px-6 text-[#1c1917] font-medium transition-all resize-none text-right">{{ old('description.ar', $service->description['ar'] ?? '') }}</textarea>
                            @error('description.ar') <p class="text-red-500 text-xs mt-2 font-bold uppercase">{{ $message }}</p> @enderror
                        </div>
                    </div>
                </div>

                <div class="border-t border-gray-50 pt-8 flex items-center justify-between" x-data="{ active: {{ $service->is_active ? 'true' : 'false' }} }">
        ...
        <div class="flex justify-end gap-6">
            <a href="{{ route('admin.services.index') }}" class="px-8 py-4 text-gray-400 hover:text-white font-bold uppercase text-xs tracking-widest transition-colors no-underline">Cancel Modification</a>
            <button type="submit"
                ...
                <span>Update Ecosystem Module</span>
