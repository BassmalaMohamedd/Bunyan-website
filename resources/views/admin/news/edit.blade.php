...
<div class="max-w-4xl mx-auto">
    <div class="mb-12">
        ...
        <h1 class="text-4xl font-black text-white tracking-tight">Refine Market Intel</h1>
        <p class="text-gray-400 mt-2 font-medium">Update the specialized intelligence specifications in the Vertex network.</p>
    </div>

    <form action="{{ route('admin.news.update', $post->id) }}" method="POST" class="space-y-8">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 gap-10 text-[#1c1917]">
                ...
                <div class="grid grid-cols-2 gap-10">
                    <div>
                        ...
                        <input name="slug" type="text" placeholder="riyadh-market-2026" value="{{ old('slug', $post->slug) }}" required
                    ...
                    <div>
                        ...
                        <input name="published_at" type="date" value="{{ old('published_at', $post->published_at ? date('Y-m-d', strtotime($post->published_at)) : '') }}"
                            class="w-full bg-gray-50 border-transparent focus:bg-white focus:border-[#f59e0b] focus:ring-0 rounded-2xl py-4 px-6 text-[#1c1917] font-bold transition-all" />
                ...
                <div>
                    ...
                    <input name="image" type="text" placeholder="/images/intel/market-analysis.jpg" value="{{ old('image', $post->image) }}"
                        class="w-full bg-gray-50 border-transparent focus:bg-white focus:border-[#f59e0b] focus:ring-0 rounded-2xl py-4 px-6 text-[#1c1917] font-bold transition-all" />
                ...
                <div class="grid grid-cols-2 gap-10 border-t border-gray-50 pt-10">
                    ...
                    <div class="space-y-6 text-left">
                        ...
                        <div>
                            ...
                            <input name="title[en]" type="text" placeholder="Riyadh Commercial Outlook" value="{{ old('title.en', $post->title['en'] ?? '') }}" required
                        ...
                        <div>
                            ...
                            <textarea name="content[en]" rows="12" placeholder="Detail the market analysis..." required
                                class="w-full bg-gray-50 border-transparent focus:bg-white focus:border-[#f59e0b] focus:ring-0 rounded-2xl py-4 px-6 text-[#1c1917] font-medium transition-all resize-none text-left">{{ old('content.en', $post->content['en'] ?? '') }}</textarea>
                    ...
                    <div class="space-y-6 text-right" dir="rtl">
                        ...
                        <div>
                            ...
                            <input name="title[ar]" type="text" placeholder="آفاق السوق التجاري في الرياض" value="{{ old('title.ar', $post->title['ar'] ?? '') }}" required
                        ...
                        <div>
                            ...
                            <textarea name="content[ar]" rows="12" placeholder="تفاصيل تقرير السوق..." required
                                class="w-full bg-gray-50 border-transparent focus:bg-white focus:border-[#f59e0b] focus:ring-0 rounded-2xl py-4 px-6 text-[#1c1917] font-medium transition-all resize-none text-right">{{ old('content.ar', $post->content['ar'] ?? '') }}</textarea>
        ...
        <div class="flex justify-end gap-6">
            ...
            <button type="submit"
                ...
                <span>Update Intelligence Module</span>
